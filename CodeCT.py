...
# 4. Thuật toán Local Search (2-opt)
def two_opt_improve(route, depot, customers):
    best = route[:]
    improved = True
    while improved:
        improved = False
        for i in range(len(best) - 1):
            for j in range(i + 2, len(best)):
                ...
                if route_distance(new_route, depot, customers) < route_distance(best, depot, customers):
                    best = new_route
                    improved = True
    return best
...
def route_distance(route, depot, customers):
    if not route:
        return 0
    d = calculate_distance(depot, customers[route[0]])
    for i in range(len(route) - 1):
        d += calculate_distance(customers[route[i]], customers[route[i+1]])
    d += calculate_distance(customers[route[-1]], depot)
    return d

def local_search_solver(depot, customers, demands, capacity):
    routes, _ = nearest_neighbor_solver(depot, customers, demands, capacity)
    improved_routes = [two_opt_improve(r, depot, customers) for r in routes]
    total_distance = sum(route_distance(r, depot, customers) for r in improved_routes)
    return improved_routes, total_distance

# 5. Vẽ biểu đồ kết quả với mũi tên cải tiến
def visualize_results(depot, customers, routes):
    ...
    plt.title("Mô phỏng lộ trình VRP - Nearest Neighbor", fontsize=15)
    plt.xlabel("Tọa độ X")
    plt.ylabel("Tọa độ Y")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if n_cust <= 0 or cap <= 0:
            ...
            # 4. Giải thuật Local Search
            ls_routes, ls_dist = local_search_solver(depot_pos, cust_pos, cust_demands, cap)

            # 5. Đánh giá hiệu suất
            print("\n" + "="*15 + " ĐÁNH GIÁ HIỆU SUẤT " + "="*15)
            print(f"| {'Thông số':<25} | {'NN':>10} | {'Local Search':>12} |")
            print("-" * 57)
            print(f"| {'Tổng quãng đường':<25} | {total_dist:>8.2f} km | {ls_dist:>10.2f} km |")
            print(f"| {'Số xe sử dụng':<25} | {len(final_routes):>10} | {len(ls_routes):>12} |")
            print(f"| {'Quãng đường TB/xe':<25} | {total_dist/len(final_routes):>8.2f} km | {ls_dist/len(ls_routes):>10.2f} km |")
            print(f"| {'Cải thiện':<25} | {'':>10} | {(total_dist - ls_dist)/total_dist*100:>10.2f} % |")
            print("=" * 57)
